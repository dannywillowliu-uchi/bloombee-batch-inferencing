...
class InferenceSession:
    ...
    def _enter_server_sessions(self, chosen_spans: List[RemoteSpanInfo]) -> List[_ServerInferenceSession]:
        ...
        try:
            for span in chosen_spans:
                ...
                # Don't add batch_size to metadata to avoid duplicate keyword argument
                # metadata["batch_size"] = self._batch_size  # REMOVED: This causes duplicate keyword argument
    ...
    def step(
        ...
        # Validate that all sequences in batch are identical
        if self._batch_size > 1:
            first_sequence = inputs[0:1]
            assert torch.allclose(inputs, first_sequence.expand(self._batch_size, -1, -1)), \
                "All sequences in batch must be identical"
    ...
    @property
    def last_token_id(self) -> Optional[torch.Tensor]:  # Backward compatibility with bloombee < 2.1.0
        return self.output_ids[:, -1:] if self.output_ids is not None else None

    @last_token_id.setter
    def last_token_id(self, value: torch.Tensor):  # Backward compatibility with bloombee < 2.1.0
